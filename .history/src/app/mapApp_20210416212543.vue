<template>
  <div style="" class="example">
    ...
    <slot v-for="(item, index) in config">
      <div v-if="item.ref === 'item1' && item.isDisplay && dataObj[item.ref]" :state="item.state" :index="index" :key="index" :class="'chartItem ' + item.ref">
      ...
      <div v-if="item.ref !== 'item1' && item.isDisplay  && dataObj[item.ref]" :index="index" :key="index" :class="'chartItem ' + item.ref">
        <DynamicItem :comName="dataObj[item.ref].comName" :title="dataObj[item.ref].title"
          :dataObj="dataObj[item.ref].dataObj" :ref="item.ref"></DynamicItem>
      </div>
    </slot>
