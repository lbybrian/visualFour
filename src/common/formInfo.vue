<template>
  <div class="FormInfoArea">
    <el-form class="formItem" :label-width="dataInfo.labelWidth" :label-position="dataInfo.labelPosition"
      :label-suffix="dataInfo.labelSuffix || ':'">
      <slot v-for="(item, index) in dataInfo.data">
        <el-form-item class="formLine" :label="item.name" :required="item.must" prop :index="index"
          style="margin-bottom: 0px">
          <el-checkbox v-if="item.type === 'checkbox'" v-model="item.checkAll" label="全部"
            :indeterminate="item.isIndeterminate" name="type" :size="item.size || 'mini'"
            @change="handleCheckAllChange(item, index)"></el-checkbox>
            ...
            <label v-for="(d, i) in item.items" :key="i" style="margin-right: 10px">
              <slot>
                <span v-if="d.type === 'checkbox' || item.type === 'checkbox'">
                  <el-checkbox @change="handleCheckedChange(item, index)" v-model="d.checked" :label="d.name || d"
                    :name="item.name" :size="item.size || 'mini'"></el-checkbox>
                </span>
                <span v-else-if="d.type === 'radio' || item.type === 'radio'">
                  <el-radio @change="dealChangeEvent(item, index)" :value="d.value" v-model="item.value"
                    :label="d.name || d" :size="item.size || 'mini'" name="type"></el-radio>
                </span>
                <span v-else-if="d.type === 'switch' || item.type === 'switch'">
                  <span v-if="d">{{ d }}</span>
                  <el-switch @change="dealChangeEvent(item, index)" v-model="item.value" active-color="#13ce66"
                    inactive-color="#ff4949" :size="item.size || 'mini'"></el-switch>
                </span>
                <span v-else-if="
                    d.type === 'dateRange' || item.type === 'dateRange'
                  ">
                  <el-date-picker type="daterange" start-placeholder="开始日期" end-placeholder="结束日期"
                    :default-time="['', '']" :size="item.size || 'mini'" v-model="item.value"
                    :picker-options="item.pickerOptions" @change="dealChangeEvent(item, index)"></el-date-picker>
                </span>
                <span v-else-if="d.type === 'tags' || item.type === 'tags'">
                  <el-tag class="eTagItem" :key="tIndex" v-for="(tag, tIndex) in d.value" :type="tag.type"
                    :size="item.size || 'mini'" style="margin-left: 5px">{{ tag.name }}</el-tag>
                </span>
                <span v-else-if="d.type === 'text' || item.type === 'text'">
                  <el-input :size="item.size || 'mini'" :placeholder="d.placeholder"
                    @change="dealChangeEvent(item, index)" v-model="item.value"></el-input>
                </span>
                <span v-else-if="d.type === 'number' || item.type === 'number'">
                  <el-input-number @change="dealChangeEvent(item, index)" :size="item.size || 'mini'"
                    :placeholder="d.placeholder" :min="d.min || item.min" :max="d.max || item.max"
                    :precision="d.precision || item.precision" v-model="item.value"></el-input-number>
                </span>
                <span v-else-if="d.type === 'select' || item.type === 'select'">
                  <el-select v-model="d.value" :size="item.size || 'mini'" @change="dealChangeEvent(d)"
                    :placeholder="d.placeholder">
                    <el-option v-for="(tItem, tIndex) in d.options" :key="tIndex" :label="tItem.label || tItem"
                      :value="tItem.value || tItem"></el-option>
                  </el-select>
                </span>
                <span v-else-if="d.type === 'textarea' || item.type === 'textarea'">
                  <el-input type="textarea" :size="item.size || 'mini'" :placeholder="d.placeholder"
                    @change="dealChangeEvent(item, index)" v-model="item.value"></el-input>
                </span>
                <span v-else-if="
                    d.type === 'datePicker' || item.type === 'datePicker'
                  ">
                  <el-date-picker v-model="item.value" @change="dealChangeEvent(item, index)"
                    :size="item.size || 'mini'" :format="d.format || item.format || 'yyyy-MM-dd HH:mm:ss'"
                    :value-format="
                      d['value-format'] ||
                      item['value-format'] ||
                      'yyyy-MM-dd HH:mm:ss'
              ...
              <slot v-if="!!d.input">
                <span v-for="(t, index) in d.input" :key="index">
                  <el-input :size="t.size || 'mini'" :placeholder="t.placeholder" v-model="item[t.mode]"
                    @change="dealChangeEvent(item, index)" :style="
          ...
          <slot v-else>
            <p style="width: 100%" v-if="
                item.value.constructor &&
                item.value.constructor.name !== 'Array'
              ">
              {{ item.value }}
            </p>
            <p v-else v-for="(t, index) in item.value" :key="index" style="width: 100%">
              {{ t }}
            </p>
          </slot>
          <slot v-if="
            ...
            <label style="
              ...
              <el-button @click="switchCondition(item, index)" type="text" size="mini">
                <i style="
          ...
          <p v-if="item.errorTips" style="line-height: 1; color: rgba(220, 0, 0, 0.8)">
            {{ item.errorTips }}
          </p>
        </el-form-item>
      </slot>
