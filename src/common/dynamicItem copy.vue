<template>
  <div class="componentArea">
    ...
    <div class="emptyTips" v-if="dataInfo.comName && currentStep === 1 && lastStatus !== 1">
      参数信息： {{dataInfo.param}} 数据接口：{{dataInfo.interface}} 可视化组件：{{dataInfo.comName}}
    </div>
    ...
    <div v-if="!dataInfo.comName" class="emptyTips">请配置该可视化组件</div>
  </div>
</template>
<script>
  ...
  export default {
    ...
    methods: {
      switchComponent(data) {
        ...
        if (this.$refs.componentItem && typeof (this.$refs.componentItem.updateData) === "function") {
